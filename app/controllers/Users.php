...
class Users extends Controller {
									...
									if ($p['password'] === $p['confirmPassword']) {
										...
										if ($this->user->rowCount() === 0) {
											...
											if ($this->user->rowCount() === 0) {
												...
												if (writeMessage($message) || mail($p['email'], 'Confirm your email', $message, 'From: noreply@example.com' . "\r\n")) {
	...
	private function sendOTP() {
		...
		if ($this->user->update(decryptAlpha($_SESSION['user'], 6), ['otp' => $otp, 'otp_sent_on' => time()])) {
			// sending OTP
			$phone = $this->user->select(decryptAlpha($_SESSION['user'], 6))->phone;
			if ((writeMessage($phone . ': ' . $otp) || sendOTP(['phone' => $phone, 'otp' => $otp]))) {
	...
	private function sendCode() {
		...
		if ($this->user->update(decryptAlpha($_SESSION['user'], 6), ['code' => $code, 'code_sent_on' => time()])) {
			// sending code
			if ($email = $this->user->select(decryptAlpha($_SESSION['user'], 6))->email && (writeMessage($message) || mail($p['email'], 'Confirm your email', $message, 'From: noreply@example.com' . "\r\n"))) {
			...
			if (postVars($p, ['email'])) {
				...
				if (validateEmail($p['email'])) {
					if (($row = $this->user->selectWhere(['email' => $p['email']])) && $this->user->rowCount() === 1) {
						...
						if (writeMessage($message) || mail($p['email'], 'Reset your password', $message, 'From: noreply@example.com' . "\r\n")) {
				...
				if (validatePassword($p['oldPassword']) && validatePassword($p['newPassword']) && validatePassword($p['confirmPassword'])) {
					...
					if ($p['newPassword'] === $p['confirmPassword']) {
						...
						if (($old = $this->user->select(decryptAlpha($_SESSION['user'], 6))->password) && password_verify($p['oldPassword'], $old)) {
							...
							if ($this->changeCredentialEncryption($p['oldPassword'], $p['newPassword'])) {
	...
	private function changeCredentialEncryption($oldKey, $newKey) {
		// getting the models
		$user = decryptAlpha($_SESSION['user'], 6);
		...
		$shared = $share->selectWhere(['shared_by' => $user]);
		if ($share->rowCount() === 1) {
			$temp[0] = $shared;
			unset($shared);
			$shared = $temp;
